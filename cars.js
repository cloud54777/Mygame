...
export class Car {
    ...
    updateApproaching(dt, lightStates) {
        // REMOVED: this.prepareForTurn() - cars should not change lanes at intersection
...
calculateTargetPosition() {
    ...
    updateWaiting(dt, lightStates) {
        ...
        if (canProceed && !carTooClose) {
            ...
            // CRITICAL FIX: Don't initialize trajectory immediately!
            // Let car move naturally to intersection entry point first
            this.trajectorySpec = null;
            this.trajectoryDistance = 0;
            this.hasInitializedTrajectory = false;
            console.log("Car", this.id, "starting to cross - will initialize trajectory when ready");
        }
        ...
        if (this.totalWaitTime > 15000) {
            ...
            this.waitStartTime = null;
            this.speed = 10;
            // ALSO reset trajectory initialization flag
            this.hasInitializedTrajectory = false;
        }
    }
